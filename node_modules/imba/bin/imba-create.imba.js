...
var require_placeholder = __commonJS({
  "node_modules/enquirer/lib/placeholder.js"(exports2, module2) {
    "use strict";
    var utils = require_utils();
    module2.exports = (prompt2, options = {}) => {
      prompt2.cursorHide();
...
// src/utils/logger.imba
var import_perf_hooks = require("perf_hooks");
var Ψspinner = /* @__PURE__ */ Symbol.for("#spinner");
var Ψctime = /* @__PURE__ */ Symbol.for("#ctime");
var ΨIMBA_OPTIONS = /* @__PURE__ */ Symbol.for("#IMBA_OPTIONS");
var Ψprefix = /* @__PURE__ */ Symbol.for("#prefix");
var Ψlast = /* @__PURE__ */ Symbol.for("#last");
var ansiMap = {
  reset: [0, 0],
  bold: [1, 22],
  dim: [2, 22],
...
function format(str, ...rest) {
  ...
  str = str.replace(/\%([\w\.]+)/g, function(m, f) {
    ...
    if (f == "markdown") {
      ...
      return fmt("green", part);
    } else if (f == "magenta") {
      return fmt("magentaBright", part);
    } else if (f == "cyan") {
      return fmt("cyanBright", part);
    } else if (f == "imba") {
...
var Logger = class {
  ...
  ts(...pars) {
    let now = import_perf_hooks.performance.now();
    let diff = this[Ψlast] ? now - this[Ψlast] : 0;
    this.write("debug", ...pars, ansi.blue("+" + diff.toFixed(1) + "ms"));
    return this[Ψlast] = now;
  }
  spinner() {
    return;
...
var Ψ__init__ = /* @__PURE__ */ Symbol.for("#__init__");
async function readΞpackage(path) {
  if (path.match(/^https?\:/)) {
    let res = await (0, import_node_fetch.default)(path);
    return res.json();
  }
  ;
  let body = import_fs.default.readFileSync(import_path.default.resolve(path, "package.json"), "utf-8");
  return JSON.parse(body);
}
function writeΞpackage(dir, data) {
  let out = JSON.stringify(data, null, 2);
  return import_fs.default.writeFileSync(import_path.default.resolve(dir, "package.json"), out);
}
var imbadir = import_path.default.resolve(__dirname, "..");
var imbapkg = readΞpackage(imbadir);
var templates = [
...
].map(function([name, hint, urlPart]) {
  return { name, hint, urlPart };
});
var cli = new class {
  constructor($$ = null) {
    this[Ψ__init__]($$);
  ...
  [Ψ__init__]($$ = null) {
    var vφ;
    this.cwd = $$ && (vφ = $$.cwd) !== void 0 ? vφ : process.cwd();
  }
  set status(value) {
    logger_default.info(value);
  }
  ok(msg, o = {}) {
    const prompt2 = new import_enquirer.Confirm(Object.assign({
      name: "question",
      message: msg
    }, o));
    return prompt2.run();
  }
  select(msg, choices, o = {}) {
    const prompt2 = new import_enquirer.Select(Object.assign({
      type: "select",
      message: msg,
      choices
    }, o));
  ...
  input(msg, o = {}) {
    const prompt2 = new import_enquirer.Input(Object.assign({ message: msg }, o));
    return prompt2.run();
  }
  exec(cmd, o = {}) {
    let res = (0, import_child_process.execSync)(cmd, Object.assign({ cwd: this.cwd }, o));
    return res.toString().trim();
  }
  package(defaults = {}) {
    const prompt2 = new import_enquirer.Snippet({
      name: "username",
...
async function run() {
  ...
  try {
    ...
    const urlPart = templates.find(function(_0) {
      return _0.name === tplname;
    }).urlPart;
    let tplurl = "https://github.com/imba/imba-" + urlPart;
    let tplpkg = await readΞpackage("https://raw.githubusercontent.com/imba/imba-" + urlPart + "/main/package.json");
    let name = process.argv[2] || "";
    name || (name = await cli.input("Name your project", { initial: "hello-imba" }));
    ...
    if (advanced) {
      ...
      if (data.repository[0] == "/") {
        φ = data.repository, delete data.repository, φ;
      }
      ;
    }
    ;
    let dir = import_path.default.resolve(data.name);
    if (await cli.ok("Create project in directory: " + dir + "?", { initial: true })) {
      logger_default.info("Generating files from template");
      cli.exec("git clone --depth 1 " + tplurl + ' "' + dir + '"');
      cli.cwd = dir;
      import_fs.default.rmSync(import_path.default.resolve(dir, ".git"), { recursive: true });
      cli.exec("git init .");
      let pkg = Object.assign({}, tplpkg, data);
      writeΞpackage(dir, pkg);
      logger_default.info("Installing dependencies");
      cli.exec("npm install imba");
      cli.exec("npm install");
      if (data.repository) {
        logger_default.info("add origin https://github.com/" + data.repository + ".git");
      ...
      return logger_default.success("Finished setting up project!\n%markdown", getStarted);
    }
    ;
  } catch (e) {
    return logger_default.error("Something went wrong during creation\n  Please report at https://github.com/imba/imba/issues");
  }
  ;
}
run();
